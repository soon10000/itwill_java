 ...
 * 
 * Object 클래스에서 모든 메서드는 하위 클래스에서 재정의(override)할 수 있음.
 *  (예) toString, equals(), hashCode(), getClass()...
 *  
 *  - toString() : 객체의 문자열 표현식을 리턴하는 메서드.
 *    패키지.클래스@주소" 형식의 문자열일 만들어서 리턴  예) com.itwill.inheritance05.T@54bedef2
 *    
 *  - equals() : 객체 동등 비교 메서드
 *    힙(heap)에 생성된 객체가 같으면 true, 다르면 false를 리턴(객체 메모리 주소가 같으면 true 아니면 false)
 *    
 *  - hashCode() : 객체의 해시코드값을 리턴
...
public class InheritanceMain05 {
	...
	public static void main(String[] args) {
		...
		// Object에서는 실제로 생성된 객체가 같으면 true를 그렇지 않으면 false를 리턴
