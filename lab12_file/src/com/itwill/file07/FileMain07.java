 ...
 *   모든 종류의 파일(txt, jpg, mp4, ppt, pdf ...)에서 사용 가능.
 * InputStream
 *  |__ FileInputStream, BufferedInputStream, ObjectInputStream, ... (읽기 단위가 byte 단위인 클래스들)
 *  
 * OutputStream
 *  |__ FileOutputStream, BufferedOutputStream, ObjectOutputStream, ... (쓰기 단위가 byte 단위인 클래스들)
 *  
 ...
 *      |__ FileWriter
 *      
 * 인코딩(encoding): 문자를 그 문자에 해당하는 코드(정수) 값으로 변환. (예) 'A' -> '65'(유니코드값)
 * 디코딩(decoding): 문자 코드(정수)를 그 코드에 해당하는 문자로 변환. (예) '65' -> 'A'           
 * 운영체제(OS)의 기본 인코딩 방식:
 *   - Unix, Linux, MacOS: UTF-8
 *   - 한글 MS Window: MS949(CP949, EUC-KR), 영문 MS Window(CP1252), ...   
 * 
 */
...
public class FileMain07 {
	...
	public static void main(String[] args) throws Exception {
		...
		// 생성자를 제공하기 시작
		// 1. FileReader 객체를 생성 할 때 인코딩 방식을 설정.
