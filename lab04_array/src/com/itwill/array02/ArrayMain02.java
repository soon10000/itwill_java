...
public class ArrayMain02 {
	...
	public static void main(String[] args) {
		...
		// 타입[] 변수이름 = {...};
		int[] nembers = {1, 3, 5, 7}; // new int[] {1, 3, 5, 7}
		...
		// 배열 원소의 값을 읽을 수는 있지만, 변경 할 수는 없다.
		for (int x : nembers) {
			System.out.println(x);		
		}
		
		// boolean 타입 2개를 저장하는 배열을 선언하고, 기본값(false)로 초기화
		...
		bools[0] = true; // 배열 bools 인덱스 0 위치의 원소를 true로 변경
		bools[4] = true; // 배열 bools 인덱스 4번 위치의 원소를 true로 변경
		for(boolean x : bools) {
			System.out.print(x + " ");
		}
		System.out.println();
